    ...
    >
      <div className="flex flex-col items-center justify-center h-screen from-pink-500 to-blue-500 text-center">
          ...
          {db.map((property, index) => (
            ...
            >
              ...
              <div className="bg-white rounded-lg overflow-hidden shadow-2xl h-full">
                <img className="h-60 w-full object-cover" src={property.img} alt={property.name} draggable={false}
                     onMouseDown={(e) => e.stopPropagation()} />
                <div className="p-6">
                  <div className="flex items-baseline">
                    <span
                      className="inline-block bg-teal-200 text-teal-800 py-1 px-4 text-xs rounded-full uppercase font-semibold tracking-wide">Online</span>
                    <div className="ml-2 mr-2 text-gray-600 text-xs uppercase font-semibold tracking-wide">&bull;</div>
                    <span
                      className="inline-block bg-red-200 text-teal-800 py-1 px-4 text-xs rounded-full uppercase font-semibold tracking-wide">Offline</span>
                  </div>
                  {/* shop name */}
                  ...
                  <div className="mt-2 flex items-center">
                    ...
                    <span className="ml-2 text-gray-600 text-sm">{property.reviews} reviews</span>
                  </div>
                </div>
              </div>
            </TinderCard>
        ...
        <div className="mt-8 flex space-x-4">
          ...
          <button
            onClick={() => swipe("left")}
            className={`w-16 h-16 flex justify-center items-center rounded-full bg-blue-500 text-white shadow-md transition-transform transform hover:scale-110 ${!canSwipe && "bg-gray-400 cursor-not-allowed"}`}
          >
            <IconCheck
              size={24}
          ...
          <button
            onClick={goBack}
            className={`w-16 h-16 flex justify-center items-center rounded-full bg-green-500 text-white shadow-md transition-transform transform hover:scale-110 ${!canGoBack && "bg-gray-400 cursor-not-allowed"}`}
          >
            <IconStar
              size={24}
              className="transition-transform transform hover:rotate-180"
            />
          </button>
          ...
          <button
            onClick={() => swipe("right")}
            className={`w-16 h-16 flex justify-center items-center rounded-full bg-red-500 text-white shadow-md transition-transform transform hover:scale-110 ${!canSwipe && "bg-gray-400 cursor-not-allowed"}`}
          >
            <IconX
              size={24}
