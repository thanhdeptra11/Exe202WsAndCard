...
const SocialMediaIcons = ({ link, platform, size = 'h-6 w-6' }) => {
  const iconComponents = {
    Instagram: (
        <svg viewBox="0 0 30 30" fill="currentColor" className={size}>
          ...
          <path
              d="M19.999,3h-10C6.14,3,3,6.141,3,10.001v10C3,23.86,6.141,27,10.001,27h10C23.86,27,27,23.859,27,19.999v-10C27,6.14,23.859,3,19.999,3z M15,21c-3.309,0-6-2.691-6-6s2.691-6,6-6s6,2.691,6,6S18.309,21,15,21z M22,9c-0.552,0-1-0.448-1-1   c0-0.552,0.448-1,1-1s1,0.448,1,1C23,8.552,22.552,9,22,9z"
          ></path>
        </svg>
    ),
    Facebook: (
  ...
  return (
      <a
          ...
          className="text-gray-500 transition-colors duration-300 hover:text-red-400 flex items-center"
      >
        ...
        for (let i = 1; i <= 5; i++) {
            stars.push(
                i <= rating ? (
                    ...
                    <IconStar key={i} className="text-gray-300"/>
                )
            );
        }
        return stars;
