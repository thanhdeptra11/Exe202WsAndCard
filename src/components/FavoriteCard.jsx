...
function FavoriteCard({ _id, name, rating, imageUrl, address, minPrice, maxPrice, email, phone, social, menu, reviews }) {
  ...
  return (
    <div className="cursor-pointer transform transition-transform duration-300 hover:scale-105 flex flex-col" onClick={handleCardClick}>
      <div className="bg-white rounded-lg overflow-hidden flex flex-col h-full ring-1 ring-gray-300 hover:ring-2 hover:ring-red-400 transform duration-200 hover:shadow-sky-200 hover:shadow-md">
        {/* Hình Ảnh Sản Phẩm */}
        <div className="relative">
          <img loading="lazy" src={imageUrl[0]} alt={name} className="w-full h-48 object-cover" />
          {/* Biểu tượng Yêu Thích (Optional) */}
          <button className="absolute top-2 right-2 bg-white bg-opacity-75 rounded-full p-2 hover:bg-opacity-100 transition">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-red-500" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" />
        ...
        <div className="p-4 flex flex-col flex-1">
          <h3 className="text-lg font-semibold text-gray-800 mb-2">{name}</h3>
          <div className="flex items-center justify-between mb-4">
            <span className="text-m text-red-500 ">{priceRange}</span>
